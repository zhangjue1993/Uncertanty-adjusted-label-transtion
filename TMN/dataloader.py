...
class dataLoader_trans(Dataset):
    """
    DataLoader for training.
    """
    def __init__(self, img_path, label_path, trans_path, augment_data=False, target_size=256):
        if os.path.exists(img_path) and os.path.exists(label_path) and os.path.exists(trans_path):
            self.inp_path = img_path
            self.out_path = label_path
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.inp_path, self.inp_files[idx])
        label_path = os.path.join(self.out_path, self.inp_files[idx])
        trans_path = os.path.join(self.trans_path, self.inp_files[idx][:-3]+'npy')
        #print(trans_path)
        ...
        return torch.from_numpy(inp_img).float(), torch.from_numpy(mask_img).long(),torch.from_numpy(transition)
