...
if __name__ == '__main__':
    ...
    model_path = os.path.join(config.TEST.model_path, 'best-model_epoch-050_mae-inf.pth')
    ...
    for image_name in image_list:
        ...
        pred_masks,logvar = model(inp_img)
        pre = pred_masks[0,1,:,:].cpu().detach().numpy()
        var = logvar[0,1,:,:].cpu().detach().numpy()
