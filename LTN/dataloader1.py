...
class dataLoader(Dataset):
    ...
    """
    def __init__(self, img_path, label_path, optimal_label_path, noise_rate,augment_data=False, target_size=256):
        if os.path.exists(img_path) and os.path.exists(label_path) and os.path.exists(optimal_label_path):
            self.inp_path = img_path
            self.out_path = label_path
            self.bayes_path = optimal_label_path
        else:
            print(img_path,label_path,optimal_label_path)
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(bayes_label_path):
